...
void DatabaseEditor::InitMenus() {
    ...
    connect(newAct, &QAction::triggered, [&]() {
        // QString filePath = QFileDialog::getSaveFileName(this, "New Database", "./archives/", "noobWarrior Database (*.nwdb)");
        // if (!filePath.isEmpty()) TryToCreateFile(filePath);
        TryToOpenFile();
    });

    connect(openAct, &QAction::triggered, [&]() {
        QString filePath = QFileDialog::getOpenFileName(this, "Open Database", "./Archives/", "noobWarrior Database (*.nwdb)");
