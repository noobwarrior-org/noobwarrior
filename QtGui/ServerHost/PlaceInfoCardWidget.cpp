...
void PlaceInfoCardWidget::LoadCarousel(EmuDb* db, int64_t placeId) {
    ...
    // The game's thumbnails in the order they were stored
    Statement stmt = db->PrepareStatement("SELECT Thumbnail, AutogeneratedThumbnailHash FROM AssetPlaceThumbnail WHERE Id = ?;");
    if (!stmt.Fail()) {
        stmt.Bind(1, placeId);
        while (stmt.Step() == SQLITE_ROW) {
            std::vector<unsigned char> bytes = !stmt.IsColumnIndexNull(1)
                ? db->DecodeImageBlob(stmt.GetStringFromColumnIndex(1))
                : db->RetrieveImageData(ItemType::Asset, stmt.GetInt64FromColumnIndex(0));
            QPixmap pm = toPixmap(bytes);
            if (!pm.isNull())
