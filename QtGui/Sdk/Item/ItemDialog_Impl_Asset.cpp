...
void ItemDialog::Asset_AddFields_Place() {
    ...
    // Populate the existing thumbnails for this place. Placeholder rows (a game with no thumbnails of
    // its own, carried as an AutogeneratedThumbnailHash blob) are auto-generated, not user-editable, so
    // they are excluded from the editor list.
    if (mId.has_value()) {
        Statement stmt = GetDatabase()->PrepareStatement("SELECT Thumbnail FROM AssetPlaceThumbnail WHERE Id = ? AND AutogeneratedThumbnailHash IS NULL;");
        stmt.Bind(1, mId.value());
