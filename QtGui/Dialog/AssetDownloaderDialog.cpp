...
AssetDownloader::AssetDownloader(QWidget *parent) {
...
void AssetDownloader::InitControls() {
    ...
    connect(mDownloadButton, &QPushButton::clicked, [&]() {
        ...
        for (int i = 0; i < mQueueEntries.size(); i++) {
            auto entry = mQueueEntries.at(i);
            auto *item = entry.at(0); // 0 because that's the index for our ID column.
...
AssetDownloader::VisualStreamBuffer::VisualStreamBuffer(QPlainTextEdit *widget) :
    mWidget(widget)
{};

auto AssetDownloader::VisualStreamBuffer::overflow(int_type c) -> int_type {
    if (c != EOF) {
...
int AssetDownloader::VisualStreamBuffer::sync() {
    mWidget->setPlainText(mWidget->toPlainText() + QString::fromStdString(mBuffer));
    mBuffer.clear();
    return 0;
}

AssetDownloader::VisualOutStream::VisualOutStream(QPlainTextEdit *widget) :
    std::ostream(new VisualStreamBuffer(widget)),
    mBuffer(dynamic_cast<VisualStreamBuffer*>(rdbuf()))
{}

AssetDownloader::VisualOutStream::~VisualOutStream() { NOOBWARRIOR_FREE_PTR(mBuffer) }