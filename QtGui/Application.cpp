...
int Application::Run() {
    ...
    ConfigResponse cfgRet = mCore->GetConfig()->ReadFromFile();
    if (cfgRet != ConfigResponse::Success) {
        std::string reasonMsg;
        switch (cfgRet) {
        default: reasonMsg = "Unknown reason"; break;
        case ConfigResponse::CantReadFile:
            reasonMsg = "Cannot read the contents of the file. Check if you have the appropriate permissions to be able to read from it.";
            break;
        ...
        case ConfigResponse::ReturningWrongType:
            reasonMsg = "Your config file is not returning a table.";
            break;
        }
        QMessageBox::critical(nullptr, "Error",
            QString("Could not read config file.\n\n%1\n\nError given by Lua: \"%2\"")
                .arg(QString::fromStdString(reasonMsg))
                .arg(QString::fromStdString(mCore->GetConfig()->GetLuaError()))
        );
