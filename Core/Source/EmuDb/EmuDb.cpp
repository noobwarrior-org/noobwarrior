...
SqlDb::Response EmuDb::AddPlaceholderThumbnailToPlace(int64_t id, const std::vector<unsigned char> &data) {
	if (Fail()) return SqlDb::Response::DatabaseFailed;

	std::string hashStr;
	SqlDb::Response res = AddBlob(data, &hashStr);
	if (res != SqlDb::Response::Success && res != SqlDb::Response::DidNothing) {
		Out("Failed to store placeholder carousel thumbnail for place id {} because the blob could not be added", id);
		return SqlDb::Response::Failed;
	}

	// A default placeholder carousel image has no backing Image asset, so it can't sit in the Thumbnail
	// column. Store the blob in AutogeneratedThumbnailHash and key the row on the place's own id, so
	// there is exactly one placeholder row per place (OR REPLACE keeps it fresh on re-backup). The
	// place-thumbnail reader treats any row with AutogeneratedThumbnailHash set as a wide placeholder.
	Statement stmt = PrepareStatement(
		"INSERT OR REPLACE INTO AssetPlaceThumbnail (Id, Thumbnail, AutogeneratedThumbnailHash) VALUES (?, ?, ?);");
	CHECK_STMT(stmt)
	stmt.Bind(1, id);
	stmt.Bind(2, id);
	stmt.Bind(3, hashStr);
	if (stmt.Step() != SQLITE_DONE) {
		Out("Failed to store placeholder carousel thumbnail for place id {}. Message: \"{}\"", id, GetLastErrorMsg());
		return SqlDb::Response::Failed;
	}
	MarkDirty();
	return SqlDb::Response::Success;
...
std::optional<EmuDb::AssetProductInfo> EmuDb::GetAssetProductInfo(int64_t id) {
	if (Fail()) return std::nullopt;

	Statement stmt = PrepareStatement(
		"SELECT a.Id, a.Name, a.Description, a.Type, a.UserId, a.GroupId, a.Created, a.Updated, "
		"a.ImageId, a.Public, h.IsNew, h.Sales, m.CurrencyType, m.Price, m.LimitedType, "
		"m.Remaining, a.MinimumMembershipLevel, a.ContentRatingTypeId "
		"FROM Asset a "
		"LEFT JOIN AssetHistorical h ON h.Id = a.Id "
		"LEFT JOIN AssetMicrotransaction m ON m.Id = a.Id "
		"WHERE a.Id = ?;");
	if (stmt.Fail()) return std::nullopt;
	stmt.Bind(1, id);
	if (stmt.Step() != SQLITE_ROW)
	...
	AssetProductInfo info;
	info.Summary.Id = stmt.GetInt64FromColumnIndex(0);
	info.Summary.Name = stmt.GetStringFromColumnIndex(1);
	info.Summary.Description = stmt.IsColumnIndexNull(2) ? "" : stmt.GetStringFromColumnIndex(2);
	info.Summary.Type = stmt.GetIntFromColumnIndex(3);
	if (!stmt.IsColumnIndexNull(4)) info.Summary.UserId = stmt.GetInt64FromColumnIndex(4);
	if (!stmt.IsColumnIndexNull(5)) info.Summary.GroupId = stmt.GetInt64FromColumnIndex(5);
	if (!stmt.IsColumnIndexNull(6)) info.Summary.Created = stmt.GetInt64FromColumnIndex(6);
	if (!stmt.IsColumnIndexNull(7)) info.Summary.Updated = stmt.GetInt64FromColumnIndex(7);
	if (!stmt.IsColumnIndexNull(8)) info.ImageId = stmt.GetInt64FromColumnIndex(8);
	if (!stmt.IsColumnIndexNull(9)) info.Public = stmt.GetIntFromColumnIndex(9) != 0;
	if (!stmt.IsColumnIndexNull(10)) info.IsNew = stmt.GetIntFromColumnIndex(10) != 0;
	if (!stmt.IsColumnIndexNull(11)) info.Sales = stmt.GetInt64FromColumnIndex(11);
	if (!stmt.IsColumnIndexNull(12)) info.CurrencyType = stmt.GetIntFromColumnIndex(12);
	if (!stmt.IsColumnIndexNull(13)) info.Price = stmt.GetInt64FromColumnIndex(13);
	if (!stmt.IsColumnIndexNull(14)) info.LimitedType = stmt.GetIntFromColumnIndex(14);
	if (!stmt.IsColumnIndexNull(15)) info.Remaining = stmt.GetInt64FromColumnIndex(15);
	if (!stmt.IsColumnIndexNull(16)) info.MinimumMembershipLevel = stmt.GetIntFromColumnIndex(16);
	if (!stmt.IsColumnIndexNull(17)) info.ContentRatingTypeId = stmt.GetIntFromColumnIndex(17);
	return info;
}
...
std::vector<unsigned char> EmuDb::RetrievePlaceThumbnailData(int64_t placeId) {
	if (Fail()) return {};

	// Prefer a creator-supplied carousel image over Roblox's wide default. The latter is represented
	// by a self-keyed row whose bytes live directly in AutogeneratedThumbnailHash.
	Statement stmt = PrepareStatement(
		"SELECT Thumbnail, AutogeneratedThumbnailHash FROM AssetPlaceThumbnail WHERE Id = ? "
		"ORDER BY CASE WHEN AutogeneratedThumbnailHash IS NULL THEN 0 ELSE 1 END, Thumbnail;");
	if (stmt.Fail()) return {};
	stmt.Bind(1, placeId);

	while (stmt.Step() == SQLITE_ROW) {
		if (!stmt.IsColumnIndexNull(1)) {
			std::vector<unsigned char> image = DecodeImageBlob(stmt.GetStringFromColumnIndex(1));
			if (!image.empty())
				return image;
			continue;
		}

		const int64_t thumbnailId = stmt.GetInt64FromColumnIndex(0);
		if (thumbnailId != 0 && thumbnailId != placeId) {
			std::vector<unsigned char> image;
			if (RetrieveDecodedAssetData(thumbnailId, 0, &image) == SqlDb::Response::Success && !image.empty())
				return image;
		}
	}
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (currentType == NoobWarrior::ItemType::Asset) {
            ...
            if (type == static_cast<int>(Roblox::AssetType::Image)) {
                ...
                // No usable raw image data, so fall through to the autogenerated thumbnail.
            }
