...
CREATE TABLE IF NOT EXISTS "AssetData" (
	...
	"Version"	INTEGER,
	"DataHash"	TEXT,
	-- Autogenerated Place/Model thumbnails normally use ImageId (Roblox gives them an internal Image
	-- asset), so this field usually isn't needed. It stays the fallback for previews that have NO
	-- associated image id: meshes/audio, and also places/models with no icon of their own - Roblox
	-- serves a rendered default placeholder for those, and that placeholder has no asset id.
