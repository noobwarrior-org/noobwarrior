...
int Database::Open(const std::string &path) {
    ...
    case 0: // 0 indicates to us that we have a brand new SQLite database, as there is no noobWarrior version that starts at 0.
        ...
        if (SetDatabaseVersion(NOOBWARRIOR_DATABASE_VERSION) != SQLITE_OK) { sqlite3_close_v2(mDatabase); return -6; }
        break;
    }

    // create all tables that do not exist
    for (int i = 0; i < NOOBWARRIOR_ARRAY_SIZE(TableNames); i++) {
        char *statement = (char*)malloc(strlen("CREATE TABLE IF NOT EXISTS '' ();") + strlen(TableNames[i]) + strlen(TableSchema[i]) + 1);
        sprintf(statement, "CREATE TABLE IF NOT EXISTS '%s' (%s);", TableNames[i], TableSchema[i]);
    ...
    for (int i = 0; i < NOOBWARRIOR_ARRAY_SIZE(MetaKv); i++) {
        char *statement = (char*)malloc(strlen("INSERT OR IGNORE INTO Meta('Name', 'Value') VALUES('', '')") + strlen(MetaKv[i][0]) + strlen(MetaKv[i][1]) + 1);
        sprintf(statement, "INSERT OR IGNORE INTO Meta('Name', 'Value') VALUES('%s', '%s')", MetaKv[i][0], MetaKv[i][1]);
        int execVal = sqlite3_exec(mDatabase, statement, nullptr, nullptr, nullptr);
        free(statement);
        if (execVal != SQLITE_OK) {
            sqlite3_close_v2(mDatabase);
