...
// Description: Ref property token; XML referents are strings, unlike the binary form's integers.
#pragma once
...
class RefToken : public IXmlPropertyToken {
public:
    std::string_view XmlPropertyToken() const override { return "Ref"; }
    ...
    void WriteProperty(const Property &property, pugi::xml_node node) const override {
        const auto *value = property.CastValue<std::string>();
        node.text().set(value != nullptr && !value->empty() ? value->c_str() : "null");
    }
};
}
