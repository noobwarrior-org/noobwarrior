...
// Description: SecurityCapabilities property token.
#pragma once
...
class SecurityCapabilitiesToken : public IXmlPropertyToken {
public:
    std::string_view XmlPropertyToken() const override { return "SecurityCapabilities"; }

    bool ReadProperty(Property &property, const pugi::xml_node &node) const override {
        uint64_t value = 0;
        if (!ParseInteger(node.text().as_string(), value))
            return false;
        property.Type = PropertyType::SecurityCapabilities;
        property.Value = SecurityCapabilities(value);
        return true;
    }

    void WriteProperty(const Property &property, pugi::xml_node node) const override {
        if (const auto *value = property.CastValue<SecurityCapabilities>())
            node.text().set(std::to_string(value->Value).c_str());
    }
};
}
