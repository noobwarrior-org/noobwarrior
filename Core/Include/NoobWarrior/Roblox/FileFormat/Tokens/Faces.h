...
// Description: Faces property token, a bitfield.
#pragma once
...
class FacesToken : public IXmlPropertyToken {
public:
    std::string_view XmlPropertyToken() const override { return "Faces"; }
    ...
    bool ReadProperty(Property &property, const pugi::xml_node &node) const override {
        ...
        if (!ParseInteger(inner.text().as_string(), value))
            return false;
        property.Type = PropertyType::Faces;
        property.Value = Faces(static_cast<uint8_t>(value));
        return true;
    }

    void WriteProperty(const Property &property, pugi::xml_node node) const override {
        if (const auto *value = property.CastValue<Faces>()) {
            node.append_child("faces").text().set(std::to_string(value->Flags()).c_str());
        }
    }
};
}
