...
public:
    ...
    // Stores Roblox's default *placeholder* carousel render (the wide GameMediaItem image a game with
    // no thumbnails of its own is served) for a place. It has no backing Image asset, so the bytes
    // live in AssetPlaceThumbnail.AutogeneratedThumbnailHash instead of as a real Image asset.
    SqlDb::Response AddPlaceholderThumbnailToPlace(int64_t id, const std::vector<unsigned char> &data);
