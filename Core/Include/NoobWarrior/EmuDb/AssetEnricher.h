...
// Description: (Disclaimer: This code was created by Claude Opus 4.8.)
...
private:
    ...
    void WorkerLoop();
    void ProcessBatch(const std::string &dbFilePath, const std::vector<int64_t> &ids, const std::string &cookie);
    ...
    std::unordered_set<int64_t> mPending; // dedupes ids already waiting in the queue
    std::string             mAuthCookie;  // snapshotted from the keychain on Enqueue (main thread)
